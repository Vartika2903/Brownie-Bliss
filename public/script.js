...
function updateFavouriteButtons(type, id) {
  ...
  document
    .querySelectorAll(`[data-fav-type="${type}"][data-fav-id="${id}"]`)
    .forEach((btn) => {
...
function addToCart(product) {
  ...
  const customKey = product.customizations
...
async function placeOrder() {
  ...
  try {
    ...
    if (data.success) {
      ...
      showToast(
        errText ||
          `Could not save order (HTTP ${res.status}). Try again or check the server.`,
      );
    }
  } catch (e) {
    console.error(e);
...
function sendWhatsAppFinal(orderId, itemsSnap, orderTotal) {
  ...
  const itemLines = lines
    .map(
      (i) =>
        `• ${i.name} × ${i.qty} = ₹${(Number(i.price) * Number(i.qty)).toLocaleString("en-IN")}`,
    )
    .join("\n");
...
function renderFavouritesPage() {
  ...
  if (bakeryGroup)
    bakeryGroup.style.display = favourites.bakeries.length ? "block" : "none";
  if (dishesGroup)
    dishesGroup.style.display = favourites.dishes.length ? "block" : "none";
}
  ...
  try {
    ...
    if (trackError) {
      trackError.textContent =
        "Error fetching order. Make sure the server is running!";
      trackError.style.display = "block";
    }
    document.getElementById("result").style.display = "none";
  }
...
function confirmCustomization() {
  ...
  const dietary =
    document.querySelector('input[name="dietary"]:checked')?.value || "egg";
  const toppings = [];
  document.querySelectorAll('input[name="topping"]:checked').forEach((cb) => {
